...
These are orthogonal failure modes. Closing one leaves the other two open. The Totem/Kick/Architect (TKA) pattern addresses each with a separate architectural layer: Totem separates execution authority from AI judgment, Kick terminates the reasoning loop from outside it, and Architect governs extraction at the seam between unstructured input and deterministic execution. TKA compliance is binary per layer, but the layers are binaries of different kinds. Totem and Kick are structural binaries: the property either holds or it does not — checkable and absolute, closed when implemented. Architect is a structural binary (an external, pre-committed novelty gate exists, or it does not) wrapped around an irreducibly probabilistic residual: near-miss false-negatives inside the OOD threshold are bounded and made auditable, not eliminated. A system that skips Totem or Kick has a known, unmitigated failure mode in full. A system that skips Architect has unconstrained, unauditable misclassification risk — the failure mode in full force.
...
Despite operating at different positions in the pipeline, all three TKA failures share a single root cause: the agent has no external ground truth to check against. We call this condition *agent dreaming*.
...
This is why **implementing two of three leaves a gap, and the gap is the third failure mode in full force.** There is no partial credit. Totem + Kick without Architect terminates cleanly and executes only through a deterministic core — on structured data that may encode a confident misreading. Totem + Architect without Kick commits only sound, governed data — until the loop producing it spins indefinitely and never reaches the core. Kick + Architect without Totem produces sound data, terminates cleanly, then lets the probabilistic component commit it directly. Each pairing is a real system with one wide-open hole.
